...
- **ไฟล์ล่าสุด**: `payment_check_v14.html` (เวอร์ชันต่อ ๆ ไปให้นับเลขต่อ อย่าทับไฟล์เก่า จะได้ rollback ได้ถ้าพัง — รูปแบบ commit ของ repo นี้คือ "rename vN เป็น vN+1" ทับของเก่าออกจาก repo ทุกครั้ง ไม่ใช่เก็บทุกเวอร์ชันไว้)
...
## แนวทางที่ลองแล้วไม่รอด (อย่าเสียเวลาทำซ้ำ)
- **ลิสต์ payments ที่ `contract_id IS NULL` (ย้ายมาจากระบบเก่า) เพื่อโชว์ "รอตรวจสอบ"** — ทำไปรอบหนึ่งแล้วถอดออก เพราะตาราง `payments` **ไม่มีคอลัมน์เก็บรหัสลูกค้าเลย** ตอนย้ายข้อมูลที่หา contract ไม่เจอ รหัสเดิมเลยหายไปจริงๆ กู้คืนจาก DB ไม่ได้ โชว์ได้แค่งวด/ยอด/วันที่ซึ่งไม่พอให้ตัดสินใจอะไร — ถ้าจะทำเรื่องนี้ใหม่ ต้องย้อนไปดูไฟล์ต้นฉบับก่อนย้ายข้อมูล (ถ้ายังมี) ไม่ใช่พึ่ง DB อย่างเดียว
- ฟีเจอร์ **"ลูกค้าที่ยังไม่มีสัญญา"** ที่ใช้แทนคือดึงจาก `computed` ของหน้า "ตรวจสอบ" (รหัสที่พิมพ์วันนี้จริงๆ) แทน — ใช้ได้จริงเพราะมีรหัสติดมาด้วย
