...
- **ไฟล์ล่าสุด**: `payment_check_v17.html` (เวอร์ชันต่อ ๆ ไปให้นับเลขต่อ อย่าทับไฟล์เก่า จะได้ rollback ได้ถ้าพัง — รูปแบบ commit ของ repo นี้คือ "rename vN เป็น vN+1" ทับของเก่าออกจาก repo ทุกครั้ง ไม่ใช่เก็บทุกเวอร์ชันไว้)
...
- `users` (user_id UUID = auth.users.id, username, nickname, is_admin, is_active, **phone, contact_email, avatar_url, theme_mode** — `theme_mode` ('light'/'dark') เก็บโหมดสว่าง/มืดผูกกับบัญชี ต่างจากรูปแบบพื้นหลัง/กระจกฝ้าที่ผูกเครื่อง) — สมัครใหม่ default `is_active=false` ต้องแอดมินอนุมัติ
...
3. **Working Reference** — ค้นหา/ดูข้อมูล + นำเข้าข้อมูล (วางจาก Excel, เทียบกับรายการวันนี้ก่อนบันทึก) + **"ย้ายลูกค้าคืนเครื่อง"** (ค้นหาสัญญา active → พรีวิวประวัติ/ยอดรวม/แผนผังงวด → ยืนยันเปลี่ยน `contracts.status` เป็น `returned`) + **"เพิ่มสัญญาใหม่"** (สร้างแถว `contracts` ผ่าน UI ได้แล้ว เดิมต้องรัน SQL มือ — ใช้ `createContract()` ที่แปล error จาก unique constraint ของ DB เป็นข้อความอ่านรู้เรื่อง) + **"ข้อมูลสัญญา" (bulk backfill)** — เลียนแบบฟีเจอร์เดียวกันในระบบเดิม: `ContractBackfillDiscovery` สแกนรหัสที่มีประวัติ payments รอตรวจสอบ (`contract_id` ว่าง + มี `device_code`) แต่ยังไม่มี `contracts` เลย เรียงจากจำนวนประวัติเยอะสุดก่อน → กด "+ กรอกสัญญา" ไปเติมโค้ดในกล่อง paste ของ `ContractBackfillCompare` (วางรหัส+จำนวนงวด+ยอดต่องวด ทีละหลายคนได้ คั่น Tab) → "เทียบข้อมูล" กับประวัติจริง (ยอดจากประวัติใช้ค่าที่จ่ายบ่อยที่สุด `mostCommonAmount`, จ่ายครบงวดถึงใช้ `maxCompletedAll` เดิม) แยกเป็น ยอดตรงกัน/ยอดไม่ตรง/ครบกำหนดสัญญาแล้ว (จะสร้างเป็น status=closed)/ยังไม่มีประวัติ/มีสัญญาเดิมอยู่แล้ว → "บันทึกลงข้อมูลสัญญา" สร้างเฉพาะแถวที่ยอดตรงกันหรือครบกำหนดแล้ว พร้อมจับคู่ payments เก่าเข้าสัญญาใหม่อัตโนมัติ — **วันที่เริ่มสัญญาเดาจากวันที่จ่ายแรกสุดในประวัติเสมอ ไม่มีให้กรอกเอง (ตามระบบเดิม)** และ **จงใจไม่ auto-overwrite ถ้ามีสัญญาเดิมอยู่แล้ว** (ต่างจากระบบเดิมที่บอกว่าจะเขียนทับ — เลือกความปลอดภัยไว้ก่อนเพราะยังไม่ได้ตกลงชัดเจน) — ฟีเจอร์ **"ลูกค้าที่ยังไม่มีสัญญา"** (`MissingContractList`/`MissingContractRow`) ของ v15/v16 **ถูกถอดออกแล้ว** เพราะแท็บนี้ทำหน้าที่แทนได้ครอบคลุมกว่า — **หมายเหตุ: ยังไม่มี flow "ย้ายลูกค้าผ่อนจบ" (`closed`) แยกต่างหาก** ทำแค่กรณีคืน/ยึดเครื่องก่อนตามที่ตกลงกัน ถ้าจะทำ flow ผ่อนจบ ให้ก็อปโครงจาก `CloseContractCard` ในไฟล์ (แค่เปลี่ยน status ที่ PATCH กับข้อความ UI)
...
7. **ตั้งค่าบัญชี (self-service)** — ทุกคนแก้ nickname/เบอร์โทร/อีเมลติดต่อ/รูปโปรไฟล์ของตัวเองได้ (รูปอัปโหลดไฟล์จริงขึ้น Supabase Storage bucket `avatars`) — อีเมลติดต่อเป็นแค่ข้อมูลติดต่อ **ไม่ใช่อีเมลล็อกอินจริง** (เปลี่ยนอีเมลล็อกอินยังไม่รองรับ) + **v16**: เพิ่ม "รูปแบบพื้นหลัง" — สลับ ปกติ/กระจกฝ้า (Liquid Glass, `applyGlass()` + `backdropFilter` blur+saturate), ปรับความทึบด้วย slider, อัปโหลดรูปพื้นหลังเอง — **ทุกอย่างในหัวข้อนี้เก็บแค่ localStorage เครื่องนั้น ไม่ผูกบัญชี/ไม่ sync ข้ามเครื่อง** (ตั้งใจแบบนี้ตามที่ตกลงกัน)
   - ปุ่มสลับโหมดสว่าง/มืดบนหัวเว็บตอนนี้ **ผูกกับบัญชี** แล้ว (คนละอย่างกับรูปแบบพื้นหลังด้านบนที่ผูกเครื่อง) — บันทึกลง `users.theme_mode` ทุกครั้งที่กดสลับ (`toggleMode()` ใน `CheckPage`), โหลดกลับมาใช้ทันทีที่โปรไฟล์โหลดเสร็จ (คีย์ effect ด้วย `profile?.user_id` กันโดนทับตอน profile object อัปเดตจากที่อื่น), ค่าเริ่มต้นก่อน login คือโหมดสว่าง
...
- **การจับคู่ payment ที่ค้างไว้กับสัญญาใหม่ (ทั้ง "เพิ่มสัญญาใหม่" และ "ข้อมูลสัญญา") ใช้ `device_code` ล้วนๆ ไม่ผูกสาขา** (เพราะ `payments` ไม่มีคอลัมน์สาขาเลย) — ถ้ารหัสเดียวกันบังเอิญไปพ้องกับอีกสาขาหนึ่ง (เช่น "9" ที่ทั้ง surat และ trang อาจมีลูกค้าคนละคนแต่ใช้รหัสเดียวกัน) จะจับคู่ผิดสาขาได้ กรณีนี้พบไม่บ่อยแต่ควรรู้ไว้ — แก้มือได้ผ่าน "ตรวจ+ซ่อมงวด" หรือ SQL ถ้าเจอ
...
`schema.sql` → `fix_identity.sql` → `rls_policies.sql` → `fix_username_trigger.sql` → `username_lookup.sql` → `account_settings.sql` → `payments_device_code.sql` → `users_theme_mode.sql`
